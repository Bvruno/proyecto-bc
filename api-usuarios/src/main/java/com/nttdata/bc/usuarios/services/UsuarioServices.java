...
public class UsuarioServices {
    ...
    return usuariosRepository
      ...
      .flatMap(usuario ->
        usuariosRepository.save(
          UsuarioEditarDto.convertToUsuarioSave(request, usuario)
        )
      )
      .switchIfEmpty(
        Mono.error(new BadRequestException("Error al guardar el usuario"))
  ...
  public Mono<ValidarTelefonoDTO.Response> validarTelefono(
    ValidarTelefonoDTO.Request request
  ) {
  ...
  public Mono<Usuario> deleteUser(String dni) {
    return usuariosRepository
      .findByDni(dni)
      .switchIfEmpty(
        Mono.error(new BadRequestException("Error al encontrar el usuario"))
      ...
      .flatMap(usuario ->
        usuariosRepository
          .delete(usuario)
          .then(Mono.just(Usuario.builder().dni(dni).build()))
      );
  }

  public Mono<Usuario> disableUser(String dni) {
    return usuariosRepository
      .findByDni(dni)
      .switchIfEmpty(
        Mono.error(new BadRequestException("Error al encontrar el usuario"))
      ...
      .flatMap(usuario ->
        usuariosRepository.save(
          Usuario.builder().dni(dni).activo(false).build()
        )
      );
  }
}
